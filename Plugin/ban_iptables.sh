...
Update_Shell(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/guaotiantang/SSRManager/master/Plugin/ban_iptables.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		[[ -z "${yn}" ]] && yn="y"
		if [[ ${yn} == [Yy] ]]; then
			wget -N --no-check-certificate https://raw.githubusercontent.com/guaotiantang/SSRManager/master/Plugin/ban_iptables.sh && chmod +x ban_iptables.sh
